...
//the integer pos state the relative position of where the <operand2> is starting
/////////////////////////////////////////////////////////////////////////////////////////
u32 imm_value(LINE_TOKEN *line_token, int pos){
    //initialising the operand
    u32 operand2 = 0;
    u32 Imm = parseExpression(line_token->operands[pos]);
    u32 Rotate = 0;
...
u32 shifting(LINE_TOKEN *line_token, int pos){
    //initialising the operand
    u32 operand2 = 0;
    operand2 += parseRegister(line_token->operands[pos]);
    if(line_token->numOfOperands > pos + 1) {
        // building the shift type
        if (strcmp(line_token->operands[pos + 1], "lsl") == 0) {

        } else if (strcmp(line_token->operands[pos + 1], "lsr") == 0) {
            operand2 |= 1 << 5;
        } else if (strcmp(line_token->operands[pos + 1], "asr") == 0) {
            operand2 |= 2 << 5;
        } else if (strcmp(line_token->operands[pos + 1], "ror") == 0) {
            operand2 |= 3 << 5;
        } else {
            printf("Error: shift type not found\n");
        }

        if (line_token->operands[pos + 2][0] == '#') {
            //if the shift value is a value
            if (parseExpression(line_token->operands[pos + 2]) > 0x1F) {
                printf("Error: shift integer too large\n");
            } else {
                operand2 |= parseExpression(line_token->operands[pos + 2]) << 7;
            }
        } else {
            //if the shift value is a register
            operand2 |= parseRegister(line_token->operands[pos + 2]) << 8;
            operand2 |= 1 << 4;
        }
    }
    return operand2;
...
/////////////////////////////////////////////////////////////////////////////////////////
// The function to assemble andeq instructions is treated as a dataprocessing instruction
// where all contents are set to 0.
/////////////////////////////////////////////////////////////////////////////////////////
void assembleAndeq(LINE_TOKEN* line_token, INSTRUCTION* instr) {
    strcpy(instr->type, "dataprocessing");
    instr->instr.dp = malloc(sizeof(DATAPROCESSING_INSTR));
    strcpy(instr->instr.dp->OPCODE, "end");
